# 🚀 MyDeusTools — Modern Windows Desktop Super-App
...
<p align="center">
  <strong>Bộ tiện ích máy tính tất cả trong một (All-in-One Utility Suite) dành cho Windows 10 & 11</strong>
</p>
...
**MyDeusTools** là ứng dụng tiện ích máy tính hiện đại, gọn nhẹ và đa năng, được thiết kế theo ngôn ngữ giao diện **Fluent Design (Mica Backdrop)** của Windows 11. Thay vì phải cài đặt nhiều phần mềm riêng lẻ nặng nề, MyDeusTools tích hợp toàn bộ các công cụ thiết yếu hàng ngày vào một ứng dụng duy nhất, có thể đóng gói thành **1 file `.exe` duy nhất** chạy ngay không cần cài đặt.
...
|---|---|
| 🖱️ **Auto Clicker Pro** | Click chuột tự động tùy chỉnh (Trái, Phải, Giữa, Đơn, Đúp), điều chỉnh khoảng thời gian (Ms/Giây/Phút/Giờ, tối thiểu 10ms), gán phím tắt toàn cục tùy ý (`NHotkey`), chế độ ghi và phát lại chuỗi tọa độ đa màn hình (`RecordingOverlayWindow`). |
| ⏱️ **Schedule Shutdown** | Lên lịch hẹn giờ Tắt máy (`Shutdown`), Khởi động lại (`Restart`), Ngủ đông (`Hibernate`) với đồng hồ đếm ngược thời gian thực định dạng `hh:mm:ss` và hỗ trợ hủy lệnh bất kỳ lúc nào. |
...
| 📋 **Clipboard Manager** | Tự động theo dõi khay nhớ tạm thời gian thực thông qua Win32 API (`AddClipboardFormatListener`), tìm kiếm nhanh nội dung đã sao chép, ghim các đoạn text quan trọng, copy lại chỉ với 1 chạm. |
...
| 🛡️ **System Tray & Khởi động cùng Windows** | Thu nhỏ ứng dụng vào khay hệ thống (Taskbar Tray Icon) khi đóng cửa sổ, chuyển đổi giao diện Sáng / Tối (Dark/Light mode) linh hoạt, tùy chọn khởi động cùng Windows qua Registry. |

---

## 🛠️ Công nghệ sử dụng

- **Ngôn ngữ & Nền tảng**: C# 12, .NET 8.0 Windows Desktop (`net8.0-windows`, `win-x64`)
...
- **Tương tác phần cứng & Hệ điều hành**:
  ...
  - `QRCoder` & `ZXing.Net` cho xử lý mã QR
...
### 3. Đóng gói ra 1 file `.exe` duy nhất (Single-File Executable)
```bash
...
## 👥 Đóng góp & Phát triển (Contributors)

Dự án được xây dựng và phát triển thông qua mô hình **Human-AI Pair Programming**:

- **[KaitoDeus](https://github.com/KaitoDeus)** — Tác giả, Kiến trúc sư & Nhà phát triển chính
- **[Claude](https://anthropic.com)** (Anthropic) — Trợ lý phát triển AI (Cộng sự kiến trúc, tối ưu mã nguồn & lập trình các module)
